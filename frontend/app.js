...
// ── STATE ────────────────────────────────────
const ob = { gender: null, activity: null, goal: null }
...
function validateObStep(step) {
  ...
  if (step === 3 && !ob.activity) return 'Please select activity level'
...
function buildSummary() {
  ...
  document.getElementById('profile-summary').innerHTML = `
    ...
    <div class="summary-item"><label>Gender</label><span>${ob.gender}</span></div>
    ...
    <div class="summary-item"><label>Activity</label><span>${actLabels[ob.activity]}</span></div>
    <div class="summary-item"><label>Goal</label><span>${goalLabels[ob.goal]}</span></div>
  `

  const base = (10 * weight) + (6.25 * height) - (5 * age) + (ob.gender === 'male' ? 5 : -161)
  ...
  const target = Math.round(base * mults[ob.activity]) + adjs[ob.goal]
...
async function loadDashboard() {
  ...
  if (profileRes.status === 'fulfilled') {
    const cal = profileRes.value.report?.target_calories
    if (cal) document.getElementById('stat-calories').textContent = cal
  }
}
...
function renderActiveProgram(program) {
  ...
  const goalLabel = { muscle_gain:'Build Muscle', fat_loss:'Burn Fat', strength:'Strength', endurance:'Endurance' }
  el.innerHTML = `
    <div style="padding:14px; background:var(--bg-3); border-radius:var(--radius-sm); border:1px solid var(--gold-border)">
      ...
      <p style="color:var(--text-3); font-size:0.82rem; margin-bottom:12px">
        ${goalLabel[program.goal] || ''} · ${program.days_per_week} days/week · ${program.exercise_count || 0} exercises
      </p>
      <button class="btn-primary" style="font-size:0.85rem; padding:8px 16px"
        onclick="showView('training'); setTimeout(() => {
...
function applyFilters() {
  ...
  if (q) list = list.filter(e => e.name.toLowerCase().includes(q) || e.muscle_group.toLowerCase().includes(q))
...
function openExModal(id) {
  ...
  const secondaryHtml = ex.secondary_muscles?.length
    ? `<div style="margin-bottom:16px">
        <p style="font-size:0.75rem; color:var(--text-3); text-transform:uppercase; letter-spacing:0.5px; margin-bottom:6px">Secondary Muscles</p>
        <div style="display:flex; gap:6px; flex-wrap:wrap">
          ${ex.secondary_muscles.map(m => `<span class="badge" style="background:var(--bg-3); color:var(--text-2); border:1px solid var(--border-2)">${m}</span>`).join('')}
        </div>
      </div>` : ''

  const instructionsHtml = ex.instructions?.length
    ? `<div style="margin-bottom:16px">
        <p style="font-size:0.75rem; color:var(--text-3); text-transform:uppercase; letter-spacing:0.5px; margin-bottom:10px">How to Perform</p>
    ...
    <div class="modal-content-body">
      ...
      ${ex.is_custom ? `
        <div style="display:flex; gap:10px; margin-top:20px; padding-top:20px; border-top:1px solid var(--border)">
          <button class="btn-secondary" onclick="editExercise(${ex.id}); closeModal()">Edit</button>
          <button class="btn-ghost-danger" onclick="deleteExercise(${ex.id}, '${ex.name}'); closeModal()">Delete</button>
        </div>
      ` : ''}
    </div>
  `
...
async function updateExercise(id) {
  ...
  const description = document.getElementById('ex-desc').value.trim()
  const required_rest_days = parseInt(document.getElementById('ex-rest').value)
  if (!name || !muscle_group || required_rest_days < 3) { showErr('exercise-form-error', 'Check all fields'); return }
  try {
    await api(`/exercises/${id}`, 'PUT', { name, muscle_group, description, required_rest_days })
    clearExForm()
    toggleForm('exercise-form')
    await loadExercises()
  } catch (err) { showErr('exercise-form-error', err.message) }
...
function switchTrainingTab(name, btn) {
  ...
  if (name === 'workout') loadWorkoutTab()
    ...
    <div class="program-card ${p.is_active ? 'is-active' : ''}">
      <div class="program-card-header">
        ...
        <div style="display:flex; gap:8px; align-items:center">
          ...
          <button class="btn-icon edit" onclick="toggleProgDetail(${p.id})">
            <i data-lucide="pencil"></i>
          </button>
          <button class="btn-icon" onclick="deleteProgram(${p.id},'${p.name}')">
      ...
      <div id="prog-detail-${p.id}" class="hidden">
        <div class="program-card-body">
          <h4 style="color:var(--gold); margin-bottom:14px; font-size:0.82rem; text-transform:uppercase; letter-spacing:0.5px">Add Exercise</h4>
          <div class="form-group">
            <label>Search</label>
            <input type="text" id="psearch-${p.id}" placeholder="Type to search..." oninput="searchProgEx('${p.id}')">
          </div>
          <div class="two-col">
            <div class="form-group">
              <label>Exercise</label>
              <select id="padd-ex-${p.id}" size="4" style="height:auto; padding:4px"></select>
            </div>
            <div class="form-group">
              <label>Day</label>
...
async function loadProgExList(progId) {
  ...
  try {
    ...
    if (!prog.exercises?.length) {
      container.innerHTML = '<p style="color:var(--text-3); font-size:0.88rem">No exercises yet.</p>'
      return
    }
        ...
        ${byDay[day].map(ex => `
          <div class="day-ex-item">
            <div>
              ...
              <span style="color:var(--text-3); font-size:0.78rem; margin-left:6px">${ex.sets}×${ex.target_reps}</span>
            </div>
...
async function createProgram() {
  ...
  if (!name || name.length < 3) {
    showErr('program-form-error', 'Name min 3 chars')
    return
  }
  try {
    await api('/programs', 'POST', {
      name,
      goal: 'strength',     // default
...
async function activateProgram(id) {
  try { await api(`/programs/${id}/activate`, 'PATCH'); await loadPrograms() }
      ...
      <div style="display:flex; justify-content:space-between; align-items:center">
        <div>
          ...
          <p style="color:var(--text-3); font-size:0.85rem">
            ${goalLabel[active.goal] || ''} · ${active.days_per_week} days/week · ${active.exercise_count || 0} exercises
          </p>
        </div>
...
async function startWorkout(progId) {
  try {
    ...
    const session = await api('/sessions', 'POST', {
      session_date, program_id: progId
    })
    activeSessionId = session.id

    const prog = await api(`/programs/${progId}`)

    // Bugünün gün numarası (1=Pazartesi ... 7=Pazar)
    const todayNum = today.getDay() === 0 ? 7 : today.getDay()

    // Bugüne ait egzersizler
    const todayExercises = (prog.exercises || []).filter(
      ex => ex.day_of_week === todayNum
    )

    // Bugün için egzersiz yoksa kullanıcıya sor
    if (todayExercises.length === 0) {
      ...
      // Bugün antrenman günü değil ama devam etmek isteyebilir
      showConfirm(
        'No workout today',
        `Today is not a scheduled training day. Load all exercises anyway?`,
        async () => {
          workoutExercises = allExercises.map(ex => ({
            ...ex, exercise_id: ex.exercise_id || ex.id
          }))
          failureState = {}
    ...
    workoutExercises = todayExercises.map(ex => ({
      ...ex, exercise_id: ex.exercise_id || ex.id
    }))
    failureState = {}
    showActiveWorkout(prog, today)
...
function renderWorkoutExercises(exercises) {
  ...
  if (!exercises?.length) {
    container.innerHTML = '<div class="empty-state"><p>No exercises in this program.</p></div>'
    return
  }
  ...
  const tabsHtml = days.map((day, i) => `
    <button class="day-tab ${i === 0 ? 'active' : ''}"
      onclick="switchDayTab(${day}, this)">
      ${dayNames[day]}
      <span class="day-tab-count">${byDay[day].length}</span>
    </button>
  `).join('')

  // Her günün egzersizleri
  const contentsHtml = days.map(day => `
    <div id="day-content-${day}" class="day-content ${day == firstDay ? '' : 'hidden'}">
      ${byDay[day].map(ex => `
        <div class="workout-ex-card">
          ...
          <div class="workout-inputs">
            <div class="form-group">
              ...
              <input type="number" id="w-kg-${ex.index}"
                placeholder="0" step="0.5" min="0"
                oninput="checkOverload(${ex.index})">
            </div>
            <div class="form-group">
              <label>Reps</label>
              <input type="number" id="w-rep-${ex.index}"
                placeholder="${ex.target_reps}" min="1">
            </div>
            <div class="form-group">
              <label>Failure?</label>
              <div class="fail-toggle">
                <button class="fail-btn" id="fy-${ex.index}"
                  onclick="setFailure(${ex.index}, true)">
                  <i data-lucide="check"></i> Yes
                </button>
                <button class="fail-btn" id="fn-${ex.index}"
                  onclick="setFailure(${ex.index}, false)">
                  <i data-lucide="x"></i> No
                </button>
              </div>
            </div>
          </div>
          <div id="overload-${ex.index}" class="overload-msg hidden"></div>
        </div>
      `).join('')}
    </div>
  `).join('')
  ...
  container.innerHTML = `
    ...
    <div class="day-contents">${contentsHtml}</div>
  `
...
async function finishWorkout() {
  ...
  for (let i = 0; i < workoutExercises.length; i++) {
    ...
    const weightEl = document.getElementById(`w-kg-${i}`)
    const repsEl = document.getElementById(`w-rep-${i}`)

    const weight = parseFloat(weightEl?.value)
    const reps = parseInt(repsEl?.value)
    const failure = failureState[i] ?? false

    // Değer girilmemişse atla
    ...
    if (!exId) {
      errors.push(`${ex.exercise_name}: missing exercise ID`)
      skipped++
      continue
    }
    ...
    try {
      ...
      errors.push(`${ex.exercise_name}: ${err.message}`)
      skipped++
    }
  }
  ...
  // Hata varsa göster
  if (errors.length > 0) {
    console.error('Workout errors:', errors)
    showToast(`Errors: ${errors.join(', ')}`, 'error', 5000)
  }

  // Hiç log olmadıysa session'ı sil
  if (logged === 0) {
  ...
  showToast(
    `Workout complete! ${logged} logged${skipped ? `, ${skipped} skipped` : ''}.`,
    'success',
    4000
  )
...
async function cancelWorkout() {
  showConfirm(
    'Cancel Workout',
    'Cancel this workout? Session will be deleted.',
    async () => {
  ...
  container.innerHTML = sessions.map(s => `
    <div class="program-card">
      <div class="program-card-header" style="cursor:pointer" onclick="toggleHistory(${s.id})">
        <div>
          <h3 style="font-size:0.95rem; margin-bottom:4px">${formatDate(s.session_date)}</h3>
...
async function loadHistoryDetail(sessionId, container) {
  try {
    ...
    if (!session.exercises?.length) {
      container.innerHTML = '<p style="color:var(--text-3); font-size:0.85rem">No exercises logged.</p>'
      return
    }
    container.innerHTML = session.exercises.map(ex => `
      <div style="display:flex; justify-content:space-between; align-items:center; padding:8px 0; border-bottom:1px solid var(--border)">
        <div>
          <p style="font-size:0.88rem; font-weight:500">${ex.exercise_name}</p>
          <p style="color:var(--text-3); font-size:0.78rem">${ex.weight_kg}kg × ${ex.reps} reps</p>
        </div>
...
async function getAISuggestion() {
  ...
  if (!days_per_week || days_per_week < 1 || days_per_week > 7) { showErr('ai-error', 'Days must be 1-7'); return }
